        ...
        <div class="min-h-screen h-full flex flex-col after:flex-1">
              ...
              <div class="max-w-md mx-auto w-full">
                <div class="relative">
                  ...
                  <ul class="relative flex justify-between w-full">
                    <li>
                      <router-link class="flex items-center justify-center w-6 h-6 rounded-full text-xs font-semibold bg-violet-500 text-white" to="/onboard">1</router-link>
                    </li>                    
                    <li>
                      <router-link class="flex items-center justify-center w-9 h-8 rounded-full text-xs font-semibold bg-violet-100 text-violet-500" to="/finish">finish</router-link>
                    </li>
                  </ul>
                </div>
              </div>
          ...
          <div class="px-4 py-8">
            <div class="max-w-md mx-auto">
              ...
              <h1 class="text-3xl text-slate-800 font-bold mb-6">User Details</h1>
              <!-- Form -->
              <form @submit.prevent="onboardUser">
                <div class="space-y-4 mb-8">
                   <!-- Full Name -->
                  <div>
                    <label class="block text-sm font-medium mb-1" for="company-name">Full Name <span class="text-rose-500">*</span></label>
                    <input id="full-name" v-model="fullName"  class="px-3 py-2 rounded border border-slate-200 shadow-sm form-input w-full" type="text" />
                  </div>
                  <!-- Company Name -->
                  <div>
                    <label class="block text-sm font-medium mb-1" for="company-name">Company Name <span class="text-rose-500">*</span></label>
                    <input id="company-name" v-model="companyName"  class="px-3 py-2 rounded border border-slate-200 shadow-sm form-input w-full" type="text" />
                  </div>
                  <!-- City and Postal Code -->
                  <div class="flex space-x-4">
                    <div class="flex-1">
                      <label class="block text-sm font-medium mb-1" for="city">City <span class="text-rose-500">*</span></label>
                      <input id="city" v-model="city" class="px-3 py-2 rounded border border-slate-200 shadow-sm form-input w-full" type="text" />
                    </div>
                    <div class="flex-1">
                      <label class="block text-sm font-medium mb-1" for="postal-code">Postal Code <span class="text-rose-500">*</span></label>
                      <input id="postal-code" v-model="postal" class="px-3 py-2 rounded border border-slate-200 shadow-sm form-input w-full" type="text" />
                    </div>
                  </div>                 
                  <!-- Country -->
                  <div>
                    <label class="block text-sm font-medium mb-1" for="country">Country <span class="text-rose-500">*</span></label>
                   <input id="country" v-model="country" class="px-2 py-2 rounded border border-slate-200 shadow-sm form-input w-full" type="text" />
                  </div>
                </div>
                <div class="flex items-center justify-between">                 
                  <button type="submit" class="px-8 py-3 font-semibold rounded bg-violet-500 text-gray-200 ml-auto" to="/finish">Finish</button>
                </div>
              </form>
...
const onboardUser = async () => {
  try {
    // Make an API request to onboard the user
     await axios.post('http://localhost:3000/api/users/onboard', {
